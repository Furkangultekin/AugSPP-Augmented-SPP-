...
% opt:   case 1: only pressure is calculated
%        case 0: all parameters are calculated
% output parameters:
%
...
if opt == 1
    pgrid  = gpt2_5(:,3:7);          % pressure in Pascal
    Tgrid  = gpt2_5(:,8:12);         % temperature in Kelvin
    Qgrid  = gpt2_5(:,13:17)./1000;  % specific humidity in kg/kg
    dTgrid = gpt2_5(:,18:22)./1000;  % temperature lapse rate in Kelvin/m
    u      = gpt2_5(:,23);           % geoid undulation in m
    Hs     = gpt2_5(:,24);           % orthometric grid height in m
    ahgrid = gpt2_5(:,25:29)./1000;  % hydrostatic mapping function coefficient, dimensionless
    awgrid = gpt2_5(:,30:34)./1000;  % wet mapping function coefficient, dimensionless
elseif opt == 0
    pgrid  = gpt2_5(:,3:7);          % pressure in Pascal
    Tgrid  = gpt2_5(:,8:12);         % temperature in Kelvin
    Qgrid  = gpt2_5(:,13:17)./1000;  % specific humidity in kg/kg
