function [ coor_rec ] = least( all,options,obs_ep )

c=299792458;%m/s;
[~,~,l_ep]=size(all);
for j=1:l_ep
    vis = all(:,1,j)~=0;
    Xsat = all(:,1,j);Ysat = all(:,2,j);Zsat = all(:,3,j);dts=all(:,4,j);
    psranges=all(:,6,j);el = all(:,8,j);ion=all(:,10,j);Trop=all(:,11,j);
    atx_cen=all(:,12,j);
    ...
    coor_rec(j,2)=c_rec(1);coor_rec(j,3)=c_rec(2);coor_rec(j,4)=c_rec(3);
    coor_rec(j,5)=dlt;
    ...
    coor_rec(j,9:11)=ENU;
    coor_rec(j,1)=obs_ep(j);
end
[ dcoor ] = dxdydz( coor_rec,options.coor );
coor_rec(:,6:8)=dcoor(:,1:3);
coor_rec(:,12)=dcoor(:,4);
end
